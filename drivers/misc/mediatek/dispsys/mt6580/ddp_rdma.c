...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle, enum RDMA_MODE mode, bool isidle)
{
  ...
  if (idx == 0)
  {
      ...
      if ( (width >= 720) || ((width>=1280) && (height<=720)) ) // HD
      {
          // Issue Reg threshold
          ...
          else // mode == RDMA_MODE_DECOUPLE
          {
              ...
              {
                  sodi_threshold = 38|(160<<10);//low:38 high:160 (decouple in HD when home screen idle senario)
                  DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 217);
